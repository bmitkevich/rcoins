...
query_pos.poloniex <- function(config=exchanges$poloniex$readonly) {
  trading_api.poloniex("returnBalances") %>% 
    map2_df(names(ret), ~ data_frame(symbol=.y, pos=as.numeric(.x))) %>% 
...
query_history.poloniex <- function(symbols, currency="BTC", start=as_datetime("2017-01-01"), stop=lubridate::now(), freq=minutes(5)) {
  symbols %>% map_df(function(s) {
    cat(s,"\n")
    d<-data_api.poloniex("returnChartData", 
...
QP <- query_quotes.poloniex() %>% filter(currency=="USD") %>% mutate(price=0.5*(bid+ask))
QP1 <- setNames(as.list(QP$price), QP$symbol)
cat("BTC",round(QP1$BTC),"ETH",round(QP1$ETH,1),"XMR",round(QP1$XMR,1))